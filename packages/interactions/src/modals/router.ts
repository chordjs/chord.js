...
import {
  Modal,
  type ModalContext,
  type ModalReplyPayload,
  type ModalDeferOptions
} from "./piece.js";
import {
  Precondition,
...
export class ModalRouter {
  ...
  get store(): Store<Modal> {
    return this.client.store(this.storeName) as Store<Modal>;
  }
  ...
  async #reply(interaction: GatewayDispatchDataMap["INTERACTION_CREATE"], payload: ModalReplyPayload): Promise<unknown> {
  ...
  async #deferReply(interaction: GatewayDispatchDataMap["INTERACTION_CREATE"], options?: ModalDeferOptions): Promise<unknown> {
    if (!this.rest) throw new Error("ModalRouter: REST client is not configured");
  ...
  async #editReply(interaction: GatewayDispatchDataMap["INTERACTION_CREATE"], payload: ModalReplyPayload): Promise<unknown> {
    if (!this.rest) throw new Error("ModalRouter: REST client is not configured");
  ...
  async #followUp(interaction: GatewayDispatchDataMap["INTERACTION_CREATE"], payload: ModalReplyPayload): Promise<unknown> {
    if (!this.rest) throw new Error("ModalRouter: REST client is not configured");
    let flags = payload.flags ?? 0;
    if (payload.ephemeral) flags |= ModalRouter.EPHEMERAL_FLAG;
