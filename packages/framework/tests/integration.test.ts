...
describe("core integration", () => {
  ...
  test("interaction router registers and executes command", async () => {
    const client = new ChordClient();
    const store = client.createStore<InteractionCommand>("commands");
  ...
  test("hybrid command routes to messageRun and chatInputRun respectively", async () => {
    const client = new ChordClient();
    const store = client.createStore<Command>("commands");
    const gateway = new MockGateway();
    const rest = new MockRest();
    ...
    class Hybrid extends Command {
      constructor(context?: any) {
        super(context || {} as any, { name: "hybrid", description: "hybrid test" });
      }
      async messageRun(): Promise<void> {
        messageCalled = true;
      }
      async chatInputRun(): Promise<void> {
        interactionCalled = true;
      }
    }

    const inst = new Hybrid();
    store.set(inst.name, inst);

    const prefixRouter = new PrefixCommandRouter({ client, prefix: "!" });
    const interactionRouter = new InteractionCommandRouter({ client, rest, storeName: "commands" });
    interactionRouter.bindGateway(gateway);

    // 1. Test Prefix
    await prefixRouter.handleMessage({ content: "!hybrid" });
